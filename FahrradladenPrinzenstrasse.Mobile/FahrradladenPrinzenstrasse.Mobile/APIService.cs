...
namespace FahrradladenPrinzenstrasse.Mobile
{
    public class APIService
    {
        ...
        public static string GetApiURL()
        {
            ...
            string api_url = "http://fp-api.p1702.app.fit.ba/api";
            return api_url;
        }
        ...
        public async Task<T> Get<T>(object search, string action = null)
        {
            ...
            catch (FlurlHttpException ex)
            {
                ...
                if (errors != null)
                {
                    ...
                    foreach (var error in errors.Extensions)
                    {
                        if (error.Value is Newtonsoft.Json.Linq.JArray array)
                        {
                            var a = array.First();
                            stringBuilder.AppendLine(string.Join(",", a.ToString()));
                        }
                        else
                            stringBuilder.AppendLine(string.Join(",", error.Value));
                    }
