...
namespace FahrradladenPrinzenstrasse.WebAPI.Services
{
    public class OpremaService : IOpremaService
    {
        ...
        public List<Model.Oprema> Get(OpremaSearchRequest request)
        {
            ...
            query = query.Where(x => x.IsDeleted == false).Where(x => x.OpremaStanje.Where(y => y.Aktivan).Where(y => y.RezervacijaProdajaOprema.Count() == 0).Any())
                .Where(x => x.Aktivan);
            ...
            foreach (var item in model_list)
            {
                ...
                var SizesQuery = _context.Oprema.Where(x => x.IsDeleted == false).Where(x => x.OpremaStanje.Where(y => y.Aktivan).Where(y => y.RezervacijaProdajaOprema.Count() == 0).Any())
                    .Where(x => x.Aktivan)
                    .AsQueryable();
        ...
        {
            var entity = _context.Oprema.Where(x => x.IsDeleted == false).Where(x => x.OpremaId == id)
