...
namespace FahrradladenPrinzenstraße.Web.Areas.Klijent.Controllers
{
    ...
    public class KorpaController : Controller
    {
        ...
        public IActionResult PotvrdaNarudzbe(PotvrdaNarudzbeVM Model)
        {
            ...
            {
                Stavke = GetKorpaStavke(Klijent),
                NaciniOtpreme = db.NacinOtpreme.OrderBy(x => x.Cijena).ToList(),
            ...
            if (Request.Method == "POST")
            {
                ...
                try
                {
                    ...
                    foreach (var naruceno_dio in VM.Stavke.Where(x => x.DioId != null).ToList())
                    {
                        ...
                        if (dio_stanja.Count < naruceno_dio.Kolicina)
                            throw new Exception("Naručeno dio " + naruceno_dio.Dio.Naziv + " nije dostupno u traženoj kolicini!");
                    ...
                    foreach (var narucena_oprema in VM.Stavke.Where(x => x.OpremaId != null).ToList())
                    {
                        ...
                        if (oprema_stanja.Count < narucena_oprema.Kolicina)
                            throw new Exception("Naručena oprema " + narucena_oprema.Oprema.Naziv + " nije dostupna u traženoj kolicini!");
                ...
                if (narudzba.NacinOtpremeId == 0)
                    narudzba.NacinOtpremeId = db.NacinOtpreme.First(x => x.Cijena == 0).NacinOtpremeId;
